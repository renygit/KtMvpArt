...
abstract class MvpFragment : MvpBaseFragment(), MvpView {
    ...
    open fun getViewReplacer() : ViewReplacer?{
        if(mViewReplacer == null) {
            val view: View? = rootView?.findViewById(R.id.viewReplacer)
            view?.let {
    ...
    open fun getLoadingView(): View {
        if(mLoadingView == null){
            if(context == null){
                throw RuntimeException("${this.javaClass.simpleName} >> MvpFragment >> getLoadingView >> context == null")
            }
    ...
    override fun getStateView():StateView{
        if(mStateView == null){
            if(context == null){
                throw RuntimeException("${this.javaClass.simpleName} >> MvpFragment >> getStateView >> context == null")
            }
    ...
    private fun useViewReplacer(@LoadState state: Int){
        ...
        if(viewReplacer != null){
            ...
            viewReplacer.currentView.setOnClickListener(if(state == STATE_LOADING || state == STATE_CONTENT) null else View.OnClickListener {
                //点击重试
                mAllPresenters.forEach { it.loadData(true) }
            })
        }else{
