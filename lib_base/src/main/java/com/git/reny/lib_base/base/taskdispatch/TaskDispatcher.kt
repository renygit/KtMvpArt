...
import com.git.reny.lib_base.base.taskdispatch.sort.TaskSortUtil
import com.git.reny.lib_base.base.taskdispatch.stat.TaskStat
...
import com.git.reny.lib_base.base.taskdispatch.task.TaskCallBack
import com.git.reny.lib_base.base.taskdispatch.utils.DispatcherLog
import com.git.reny.lib_base.base.taskdispatch.utils.Utils
import java.util.*
...
class TaskDispatcher private constructor() {
    ...
    private val mFutures: MutableList<Future<*>> =
        ArrayList()
    ...
    private val mNeedWaitCount =
        AtomicInteger() //保存需要Wait的Task的数量
    private val mNeedWaitTasks: MutableList<Task> =
        ArrayList<Task>() //调用了await的时候还没结束的且需要等待的Task

    @Volatile
    private var mFinishedTasks: MutableList<Class<out Task?>> =
        ArrayList<Class<out Task?>>(100) //已经结束了的Task
    private val mDependedHashMap: HashMap<Class<out Task?>, ArrayList<Task>?> =
        HashMap<Class<out Task?>, ArrayList<Task>?>()
    private val mAnalyseCount =
        AtomicInteger() //启动器分析的次数，统计下分析的耗时；
        ...
        if (task.dependsOn() != null && task.dependsOn()!!.isNotEmpty()) {
            for (cls in task.dependsOn()!!) {
                if (mDependedHashMap[cls] == null) {
                    mDependedHashMap[cls] = ArrayList<Task>()
                }
    ...
    fun start() {
        ...
        if (mAllTasks.size > 0) {
            ...
            printDependedMsg()
            mAllTasks = TaskSortUtil.getSortResult(mAllTasks, mClsAllTasks) as MutableList<Task>
    ...
    fun satisfyChildren(launchTask: Task) {
        val arrayList: ArrayList<Task>? = mDependedHashMap[launchTask.javaClass]
